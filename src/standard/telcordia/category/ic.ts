...
import { 
    TelcordiaAnalogICSpecifications, 
    TelcordiaDigitalICSpecifications, 
    TelcordiaRAMICSpecifications, 
    TelcordiaROMICSpecifications, 
    TelcordiaMicroprocessorSpecifications, 
    TelcordiaBasicSpecifications
} from "../specifications";

// IC Specifications - Tables 8-5 to 8-15
...
export type TelcordiaMicroprocessorSpecifications = {
    numberOfGates?: number; // for Bipolar/NMOS
    busWidth?: number; // for CMOS (bits)
    technology: 'Bipolar' | 'NMOS' | 'CMOS';
}

export const TelcordiaAnalogIC: TelcordiaCategory = {
    id: "SR332-IC-ANALOG",
    name: "Analog IC",
    mainCategory: TelcordiaMainCategories.IC,
    g_fr: (args?: Record<string, any>) => {
        ...
        if (transistors <= 860) {
            ...
            if (transistors <= 32) return 1.2;
            if (transistors <= 90) return 2.1;
            if (transistors <= 170) return 3.0;
            if (transistors <= 260) return 3.4;
            if (transistors <= 360) return 4.1;
            if (transistors <= 470) return 4.9;
            if (transistors <= 590) return 5.3;
            if (transistors <= 720) return 5.9;
            if (transistors <= 860) return 6.2;
        }
    ...
    g_std: (args?: Record<string, any>) => {
        ...
        if (transistors <= 860) {
            // Use lookup table for standard ranges from Table 8-5
            if (transistors <= 32) lambdaG = 1.2;
            else if (transistors <= 90) lambdaG = 2.1;
            else if (transistors <= 170) lambdaG = 3.0;
            else if (transistors <= 260) lambdaG = 3.4;
            else if (transistors <= 360) lambdaG = 4.1;
            else if (transistors <= 470) lambdaG = 4.9;
            else if (transistors <= 590) lambdaG = 5.3;
            else if (transistors <= 720) lambdaG = 5.9;
            else if (transistors <= 860) lambdaG = 6.2;
            else lambdaG = 0.33 * Math.pow(transistors, 0.440);
        } else {
    ...
    specificationList: TelcordiaAnalogICSpecifications
}

export const TelcordiaDigitalIC: TelcordiaCategory = {
    id: "SR332-IC-DIGITAL",
    name: "Digital IC",
    mainCategory: TelcordiaMainCategories.IC,
    g_fr: (args?: Record<string, any>) => {
    ...
    tempCurve: (args?: Record<string, any>) => {
        const tech = args?.technology || 'CMOS';
        switch(tech) {
            case 'Bipolar': return TelcordiaTempStressCurves._6;
            case 'NMOS': return TelcordiaTempStressCurves._8;
            case 'CMOS': return TelcordiaTempStressCurves._8;
            default: return TelcordiaTempStressCurves._8;
        }
    },
    elecCurve: () => 1,
    specificationList: TelcordiaDigitalICSpecifications
}

export const TelcordiaRAM: TelcordiaCategory = {
    id: "SR332-IC-RAM",
    ...
    tempCurve: (args?: Record<string, any>) => {
        ...
        switch(tech) {
            case 'Static_Bipolar':
                return density > 0.00224 ? TelcordiaTempStressCurves._6 : TelcordiaTempStressCurves._7;
            case 'Static_NMOS':
            case 'Static_CMOS':
            case 'Dynamic_NMOS':
            case 'Dynamic_CMOS':
                return density > 0.038 ? TelcordiaTempStressCurves._8 : TelcordiaTempStressCurves._9;
            default:
                return TelcordiaTempStressCurves._8;
        }
    },
    elecCurve: () => 1,
    specificationList: TelcordiaRAMICSpecifications
}

export const TelcordiaROM: TelcordiaCategory = {
    id: "SR332-IC-ROM",
    ...
    tempCurve: (args?: Record<string, any>) => {
        ...
        switch(tech) {
            case 'Bipolar':
                return TelcordiaTempStressCurves._6;
            case 'NMOS':
            case 'CMOS':
                return density > 38 ? TelcordiaTempStressCurves._10 : TelcordiaTempStressCurves._9;
            default:
                return TelcordiaTempStressCurves._9;
        }
    },
    elecCurve: () => 1,
    specificationList: [...TelcordiaBasicSpecifications, ...TelcordiaROMICSpecifications]
}

export const TelcordiaMicroprocessor: TelcordiaCategory = {
    ...
    elecCurve: () => 1,
    specificationList: [...TelcordiaBasicSpecifications, ...TelcordiaMicroprocessorSpecifications]
}

export const TelcordiaICCategories: TelcordiaCategory = {
    id: "SR332-IC",
    name: "IC (Integrated Circuit)",
    mainCategory: TelcordiaMainCategories.IC,
    subCategories:[TelcordiaAnalogIC, TelcordiaDigitalIC, TelcordiaRAM, TelcordiaROM, TelcordiaMicroprocessor]
};