export class GammaDistribution {
    static gamma(x: number): number {
       ...
       * 감마 분포의 CDF
       */
      static gammaCDF(x: number, shape: number, scale: number = 1): number {
          if (x <= 0) return 0;
          if (shape <= 0 || scale <= 0) {
       ...
       */
      private static gammaSeries(a: number, x: number, epsilon: number = 1e-15): number {
          let sum = 1 / a;
          let term = 1 / a;
          let n = 1;
       ...
       */
      private static gammaContinuedFraction(a: number, x: number, epsilon: number = 1e-15): number {
       ...
       */
      static gammaInverseCDF(p: number, shape: number, scale: number = 1): number {
          if (p < 0 || p > 1) {
       ...
       * 감마 분포의 PDF
       */
      static gammaPDF(x: number, shape: number, scale: number = 1): number {
          if (x < 0) return 0;
          if (shape <= 0 || scale <= 0) {
              throw new Error('Shape and scale parameters must be positive');
          }
       ...
       */
      private static wilsonHilfertyApproximation(p: number, shape: number): number {
       ...
       */
      private static gammaInverseCDFBisection(p: number, shape: number, scale: number): number {
          let low = 0;
...
export class NormalDistribution {
     ...
     * @returns 분위수 값
     */
    static normalInverseCDF(p: number, shape: number = 0, scale: number = 1): number {
        if (p <= 0 || p >= 1) {
     ...
     * 별칭: 역함수와 동일한 기능
     */
    static normalQuantile(p: number, mean: number = 0, stdDev: number = 1): number {
